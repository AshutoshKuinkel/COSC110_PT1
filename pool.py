...
def get_cheapest_cost(no_of_adults,no_of_children) -> int:
  '''
  Write up docstring for this helper
  '''
  ...
  for num_of_pass_a in range(all_possible_nums_of_family_a_passes):
    for num_of_pass_b in range(all_possible_nums_of_family_b_passes):
      ...
      adults_remaining = no_of_adults - adults_used_for_pass_a - adults_used_for_pass_b
      children_remaining = no_of_children - children_used_for_pass_a - children_used_for_pass_b
      ...
      # only store minimum cost out of all the costs we calulated:
...
def main() -> None:
  while(determine_if_sale_called != "q"):
    ...
    get_cheapest_cost(no_of_adults,no_of_children)
